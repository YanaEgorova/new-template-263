export const item = product => {
  return `
    <li class="product__item item">
      ...
      <p class="product__name js_product-name"><span class="product__span">Name:</span>${product.name}</p> 
      ${
        product.description[0].text.length > 0
          ? '<p class="text product__desc"><span class="product__span">Description:</span>' +
            product.description[0].text.slice(0, 100) +
            '...' +
            '</p>'
          : ''
      }
      <span class="product__price"><span class="product__span">Price:</span>$${product.price.toFixed(2)}</span>
